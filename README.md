...
書籍では、IFからWDまで一気通貫で実装しているが、同じ手順で実施してみたらどハマりした。

そのためこのレポジトリでは以下のフローで、少しずつ手を進めた体で記録する

* [x] IFとそれ以外のステージでのパイプライン化 (IF - それ以外)
  * `IF <-> EXE`間で分岐ハザードが発生しうるので`Ch.24`の無効化処理を先行対応する
  * ~~`IF <-> MEM`間と、`IF <-> WD`間のデータハザードは発生しうるので、`Ch.25`のフォワーディングも応する~~
    * ↑起きなかった・・・次フローに先送り
  * ソースは`src/main/scala/16a_pipeline`に置く
* [ ] WDをパイプラインとして切り出す (IF - ID〜MEM - WD)
  * `IF <-> WD`間のデータハザードは発生しうるので、`Ch.25`のフォワーディングも応する
* [ ] MEMをパイプラインとして切り出す (IF - ID〜EXE - MEM - WD)
  * `IF <-> MEM`間のデータハザードは発生しうるので、`Ch.25`のフォワーディングも応する
* [ ] すべてのステージをパイプライン化 (IF - ID - EXE - MEM - WD)
  * `ID <-> EXE`間のデータハザードは発生しうるので`Ch.25`のストール処理も対応する

各フローについて`rv32ui-p`系の`riscv-tests`を回す。
